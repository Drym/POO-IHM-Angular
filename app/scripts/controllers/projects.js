...
angular.module('pooIhmExemplesApp')
  .controller('ProjectsCtrl', ['$scope', '$http', '$routeParams', '$location', function ($scope, $http, $routeParams, $location) {
    ...
    //Ajouter un utilisateur a un projet (dans show)
    $scope.addUserToProjectFast = function(id) {
      $http.put('http://poo-ihm-2015-rest.herokuapp.com/api/Projects/' + $scope.currentProject.id + '/Users/' + id)
        .success(function(data) {
    ...
    $scope.delUserToProjectFast = function(id) {
      $http.delete('http://poo-ihm-2015-rest.herokuapp.com/api/Projects/' +  $scope.currentProject.id + '/Users/' + id)
        .success(function(data) {
          $location.path('/projects/'+ $scope.currentProject.id );
        });
    }
